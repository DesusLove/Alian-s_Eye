...
def _fetch_avatar(url: str, timeout: float = 6.0) -> io.BytesIO | None:
    if not url:
        return None
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 alien's-eye"})
...
def write_pdf(path: str | Path, results_data: dict[str, Any]) -> None:
    ...
    doc = SimpleDocTemplate(str(path), pagesize=A4, title="Alien's Eye Report")
    story: list[Any] = []

    summary = results_data.get("scan_summary", {})
    story.append(Paragraph("Alien's Eye — OSINT Report", styles["Title"]))

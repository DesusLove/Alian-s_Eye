...
<head>
<meta charset="utf-8">
<title>Alien's Eye Report</title>
<style>
...
"""
    ...
    def _write_pdf(self, path: Path, results_data: dict[str, Any]) -> bool:
        ...
        try:
            from alians_eye.core.pdf_report import write_pdf
        except ImportError:
            self.console.print(
                "[yellow]PDF export needs reportlab: pip install \"alien's-eye\\[pdf]\"[/yellow]"
            )
            return False

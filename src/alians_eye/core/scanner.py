...
from alians_eye.utils.console import ScanView, get_console
...
def load_sites_data(path: Path | None = None) -> dict[str, str]:
    ...
    text = (resources.files("alians_eye.data") / "sites.json").read_text("utf-8")
...
def load_site_plugins(extra_dirs: list[Path] | None = None) -> dict[str, str]:
    ...
    dirs = [Path("sites.d"), Path(user_config_dir("alians_eye")) / "sites.d"]
...
def load_nsfw_sites() -> list[str]:
    ...
    try:
        text = (resources.files("alians_eye.data") / "nsfw_sites.json").read_text("utf-8")
        return json.loads(text)

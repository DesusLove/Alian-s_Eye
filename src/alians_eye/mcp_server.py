"""MCP server exposing Alien's Eye as tools for LLM agents (optional ``[serve]`` extra: mcp).
...
Start it with ``alians_eye serve`` and point an MCP client (e.g. Claude) at the
command.
"""
...
async def _run_scan(username: str, level: str, sites: str | None, no_ml: bool) -> dict[str, Any]:
    ...
    from alians_eye.core.config import ScannerConfig
    ...
    from alians_eye.core.scanner import UsernameScanner, load_sites_data
    from alians_eye.utils.console import set_plain
...
async def serve(args) -> None:
    try:
        ...
        from alians_eye.utils.console import get_console

        get_console().print(
            "[yellow]The MCP server needs the mcp package: pip install \"alien's-eye\\[serve]\"[/yellow]"
        )
        return

    sites = getattr(args, "sites", None)
    no_ml = getattr(args, "no_ml", False)
    mcp = FastMCP("alien's-eye")
    ...
    def read_report(path: str) -> dict[str, Any]:
        """Load a previously saved Alien's Eye report JSON from disk."""
        from alians_eye.core.report import ReportError, load_report

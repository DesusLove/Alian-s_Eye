...
Requires the ``train`` extra: ``pip install alien's-eye[train]``.
...
def train_model(dataset_path: Path, output_path: Path, logger=None) -> dict:
    try:
        ...
        raise RuntimeError(
            "Training requires scikit-learn. Install with: pip install alien's-eye[train]"
        ) from exc

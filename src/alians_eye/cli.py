"""Command-line interface for Alien's Eye."""
...
from alians_eye import __version__
from alians_eye.core.analyzer import FeatureExtractor
from alians_eye.core.browser import BrowserFallback
from alians_eye.core.config import DEFAULT_USER_AGENT, ScannerConfig
from alians_eye.core.detector import Detector
from alians_eye.core.exporter import ResultsExporter
from alians_eye.core.fingerprints import FingerprintStore
from alians_eye.core.scanner import (
    UsernameScanner,
    filter_sites,
    load_nsfw_sites,
    load_site_plugins,
...
from alians_eye.utils.console import get_console, set_plain
from alians_eye.utils.logger import setup_logger
...
def build_parser() -> ArgumentParser:
    ...
    parser.add_argument("-V", "--version", action="version", version=f"alien's-eye {__version__}")
    ...
    parser.add_argument(
        "--playwright",
        action="store_true",
        help="Enable Playwright fallback for Maybe results (requires alien's-eye[browser])",
    )
...
def seed_usernames_from_lookups(args) -> list[str]:
    ...
    from alians_eye.core.variations import usernames_from_email, usernames_from_name
...
def load_config(path_value: str | None, logger) -> dict:
    if path_value:
        ...
        candidates = [
            Path("config.json"),
            Path(user_config_dir("alians_eye")) / "config.json",
        ]
...
async def run_scan(args) -> None:
    ...
    checkpoint = None
    if args.resume:
        from alians_eye.core.checkpoint import Checkpoint
    ...
    if args.watch:
        from alians_eye.core import watch as watch_mod
    ...
    try:
        for username in usernames:
            ...
            correlation = None
            if args.correlate:
                from alians_eye.core.correlate import correlate_report
            ...
            domains = None
            if args.domains:
                from alians_eye.core.domains import check_domains
...
async def _recurse_scan(scanner, all_results, base_username, scan_level, depth, console) -> None:
    ...
    from alians_eye.core.expand import candidate_usernames_from_results
...
def _print_clusters(console, correlation: dict) -> None:
    ...
    if not correlation.get("avatar_hashing"):
        console.print(
            "[dim]Tip: install \"alien's-eye\\[correlate]\" for avatar-image matching.[/dim]"
        )
...
async def run_selfcheck_command(args) -> None:
    from alians_eye.selfcheck import run_selfcheck
...
async def run_train_command(args) -> None:
    ...
    if args.train_command == "collect":
        from alians_eye.ml.collect import collect_dataset
        ...
        from alians_eye.ml.train import train_model
...
def run_diff_command(args) -> None:
    from alians_eye.core.report import ReportError, diff_reports, load_report
...
async def run_serve_command(args) -> None:
    from alians_eye.mcp_server import serve
...
async def run_label_command(args) -> None:
    from alians_eye.ml.label import label_report
